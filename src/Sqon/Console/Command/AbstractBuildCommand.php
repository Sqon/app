...
<fg=red># Sqon build configuration settings.
sqon:

    # The custom PHP bootstrap script.
    bootstrap: 'custom/bootstrap.php'

    # The compression mode (BZIP, GZIP2, NONE).
    compression: GZIP

    # The path to the main script in the Sqon.
    main: 'bin/sqon'

    # The path to create the new Sqon at.
    output: 'sqon'

    # The paths to set in the Sqon.
    paths: ['.']

    # The shebang line for the default PHP bootstrap script.
...
HELP
    ...
    private function parseConfig(array $settings, $file)
    {
        ...
        return [
            dirname($file),
            [
                'sqon' => array_merge($data['sqon'], $settings['sqon'])
            ]
        ];
    }
